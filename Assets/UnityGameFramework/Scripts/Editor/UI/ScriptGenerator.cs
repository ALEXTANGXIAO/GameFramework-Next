...
public class ScriptGenerator
{
    ...
    [MenuItem("GameObject/ScriptGenerator/ActorName", priority = 49)]
    public static void GeneratorActorName()
    {
        Generate(false, true);
    }
    ...
    private static void Generate(bool includeListener, bool isActorName = false)
    {
        var root = Selection.activeTransform;
        if (root != null)
        {
            ...
            Ergodic(root, root, ref strVar, ref strBind, ref strOnCreate, ref strCallback, isActorName);
    ...
    private static void Ergodic(Transform root, Transform transform, ref StringBuilder strVar, ref StringBuilder strBind, ref StringBuilder strOnCreate, ref StringBuilder strCallback, bool isActorName)
    {
        for (int i = 0; i < transform.childCount; ++i)
        {
            Transform child = transform.GetChild(i);
            WriteScript(root, child, ref strVar, ref strBind, ref strOnCreate, ref strCallback, isActorName);
            if (child.name.StartsWith("m_item"))
            {
                // 子 Item 不再往下遍历
                continue;
            }
            Ergodic(root, child, ref strVar, ref strBind, ref strOnCreate, ref strCallback, isActorName);
        }
    }
        ...
        foreach (var pair in dicWidget)
        {
            ...
            if (varName.StartsWith(key))
            {
                ...
                if (isActorName)
                {
                    if (key == "m_text")
                    {
                        varType = "ActorNameBinderText";
                    }
                    else if (key == "m_eff")
                    {
                        varType = "ActorNameBinderEffect";
                    }
                }
                break;
            }
